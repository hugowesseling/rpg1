...
public class ThrowRocksRandomlyAction implements ObjectAction, Serializable {
	...
	public boolean doActionAndCheckIfDone(WorldState worldState, LevelState levelState) {
		...
		if(timer<0){
			...
			levelState.gameObjectsToAdd.add(new BouncingRock(gameObject.getPosition().clone().addToThis(gameObject.getDirection().movement.multiply(10)), gameObject.getDirection().movement.multiply(3)));
		}
		return false;
	}
